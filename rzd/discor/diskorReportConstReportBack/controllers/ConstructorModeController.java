...
public class ConstructorModeController extends AnchorPane {
    ...
    public ConstructorModeController() throws Exception {
        setFullFormInfo(null, null, null);
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/ConstructorMode.fxml"));
    ...
    public void SetCurrentFileActiveSheet(){
        ...
        //System.out.println("CurrentActiveSheet = " + CurrentFileActiveSheet);
    }
...
//        stage.setResizable(false);
        ...
        chooseFormController.setInitialParams(oldObjGroup, oldForm, ProjectMode.constructor);
        stage.showAndWait();
        SetCurrentFileActiveSheet();
        System.out.println(generalParamsService.getListNumericalPokValue());
    }
    ...
    void updateFormDisplayedInfo() {
        if (this.form.getValue() == null) {
            this.stage.setTitle("Конструктор отчетов. Режим конструктора");
            this.MenuSave.setDisable(true);
            this.MenuFormInfo.setDisable(true);
            this.MenuDelete.setDisable(true);
        } else {
            //Установка нового заголовка окна с именем выбранной формы и группы
            this.stage.setTitle("Конструктор отчетов. Режим конструктора (Форма \"" + this.form.getValue().getName() + "\", группа \"" + this.objGroup.getValue().getName() + "\")");
...
//        stage.setResizable(false);
        ...
        chooseObjGroupController.setInitialParams(oldObjGroup, ProjectMode.constructor);
        stage.showAndWait();
    }
    ...
    private void PressedButtonCutOut() throws Exception {
        ...
        if (checkButtonMarkUpIsDisable()) {
            ...
            createInfoNotification("Для операции 'Вырезать' необходимо выделить не меньше\nодной размеченной ячейки");
            return;
        }
    ...
    private void PressedChangeMode() {
        try {
            Main.openOperatorMode(this.stage);
        } catch (Exception e) {
            e.printStackTrace();
            createAlertWarning(this.stage, e.getMessage());
        }
